...
from users.models import User, SecurityGroup
...
class IncidentViewSet(APIView):
    ...
    def get(self, request):
        ...
        if id != None:
            ...
            # Grabs the user from the JWT Token and serializes it for security groups
        ...
        if id != None:
            ...
            if (user.role_id == 1):
                ...
                if(str(userSecGroup.data['security_group'][0]) == str(serviceDesk.data['securityGroupId'])):
                    ...
                    if (serializer.data['security_group'] != None):
                        isTrue = False
                        # The for loops through the security groups, and the if sees if the security group of the ticket matches that of the user
                        for serializer.data['security_group'] in userSecGroup.data['security_group']:
                            if serializer.data['security_group'] in userSecGroup.data['security_group']:
                                isTrue = True
                        if (isTrue == True):
            ...
            elif (user.role_id == 3 and (str(user.id) == str(serializer.data['ticketOwnerId']))):
                return Response(serializer.data)
        ...
        if (user == 2):
            ...
            if(str(userSecGroup.data['security_group'][0]) == str(serviceDesk.data['securityGroupId'])):
                ...
                incidentData = Incident.objects.raw(
                    'SELECT * FROM incidents_incident i WHERE "ticketOwnerId_id"::uuid = %s OR "assignedTechId_id"::uuid = %s OR i.security_group_id IN (SELECT s.securitygroup_id FROM users_user_security_group s WHERE s.user_id = %s);', [userId, userId, userId])
