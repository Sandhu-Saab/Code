...
/**
 * Returns the section class list based on the instructor's section.
 * 
 * @returns The section class list based on the instructor's section.
 */
function ClassDataGridInstructor() {
  const [users, setClassList] = useState([]);
  ...
  /**
   * Fetch all users from the same section and store it on Users state.
   */
  function getUsers() {
    axios({
      method: "GET",
      url: `/api/viewClassListForInstructor/`,
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      },
   ...
   */
   function getClassRoleValue(params) {
    ...
    roles.forEach((role) => {
      if (role.roleId === Number.parseInt(params.row.role)) {
        value = role.name;
      }
    });
  ...
  function getSectionValue(params) {
    ...
    sections.forEach((section) => {
      if (params.row.course_id === String(section.id)) {
...
export default ClassDataGridInstructor;