...
const steps = ["Open", "Pending", "Work in Progress", "Resolved", "Closed"];
...
function ViewIncidentForm({
     ...
     * has permissions to view the specified incident ticket.
     */
    const verifyUser = () => {
        let viewIncidentUser = incidentData.ticketOwnerId;
        let viewIncidentRole = incidentData.ticketOwnerRole;
        let viewIncidentSection = incidentData.ticketOwnerSection;
            
        // Instructor
        if (currentUserRoleId === "3") {
          // Instructor is viewing another instructor/student asset ticket from a different section.
          if (currentUserSection !== viewIncidentSection) {
        ...
        if (currentUserRoleId === "4") {
            // Student is viewing another admin/technician/instructor/student incident ticket from a different section.
            if (currentUserSection !== viewIncidentSection) {
                navigate("/incident/all");
            }

            // Student is viewing another instructor/student tickets from the same section
            if (currentUserSection === viewIncidentSection && Number.parseInt(currentUserId) !== viewIncidentUser) {
                navigate("/incident/all");
            }
        }
    }
    ...
    return (
        <>
            <Box pb={5} sx={{ width: "100%" }}>
                <Stepper alternativeLabel activeStep={Number(incidentData.status) - 1}>
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={4}>
                            ...
                            <TextField
                                ...
                                margin="normal"
                                fullWidth
                                value={getSecurityGroup(incidentData.SecurityGroup)}
                                name="securityGroup"
                                inputProps={{ readOnly: true }}
                            />
